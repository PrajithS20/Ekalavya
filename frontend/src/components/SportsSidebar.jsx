...
import {
    LayoutDashboard,
    FlaskConical, // Career Guidance
    Dumbbell, // Training Log
    MessageSquare, // Team Chat
    Activity, // My Stats
    Utensils, // Nutrition
    MapPin, // Venues
    Trophy, // Scouting
...
const navItems = [
    { label: "Dashboard", icon: LayoutDashboard, path: "/sports" },
    { label: "Career Guidance", icon: FlaskConical, path: "/sports/guidance" },
    { label: "Training Log", icon: Dumbbell, path: "/sports/training" },
    { label: "Team Chat", icon: MessageSquare, path: "/sports/chat" },
    { label: "My Stats", icon: Activity, path: "/sports/stats" },
    { label: "Nutrition", icon: Utensils, path: "/sports/nutrition" },
    { label: "Venues", icon: MapPin, path: "/sports/venues" },
    { label: "Scouting", icon: Trophy, path: "/sports/scouting" },
];

export default function SportsSidebar({ onLogout }) {
                    ...
                    >
                        <div className="relative z-10 flex flex-col h-full">
                            <div className="flex items-center justify-between mb-10">
                                ...
                                >
                                    <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-emerald-400 to-green-600 flex items-center justify-center shadow-lg shadow-emerald-900/20">
                                    ...
                                    <motion.div
                                        ...
                                        exit={{ opacity: 0, x: -10 }}
                                    >
                                        <h1 className="text-white text-xl font-bold tracking-widest group-hover:text-emerald-400 transition-colors">
                                            EKALAVYA
                                        </h1>
                                    </motion.div>
                                </motion.div>
                                ...
                                {navItems.map((item, index) => (
                                    ...
                                    >
                                        {location.pathname === item.path && (
                                            <div className="absolute left-0 top-0 bottom-0 w-1 bg-emerald-500 shadow-[0_0_10px_2px_rgba(16,185,129,0.5)]"></div>
                                        )}
                                        <item.icon
                                            size={18}
                                            className={`transition-transform duration-300 ${location.pathname === item.path ? "scale-110 drop-shadow-[0_0_5px_rgba(16,185,129,0.5)]" : "group-hover:scale-110"}`}
                                        />
                                ...
                                >
                                    ...
                                    <div>
                                        <p className="font-bold text-white text-sm group-hover:text-emerald-400 transition-colors">
                                            {sessionStorage.getItem("userName") || "Athlete"}
                                        </p>
                                        <p className="text-xs text-gray-400">Level 1 Scout</p>
                                    </div>
                                </div>
